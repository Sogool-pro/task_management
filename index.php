...
if (isset($_SESSION['role']) && isset($_SESSION['id'])) {
    ...
    if ($_SESSION['role'] == "admin") {
        ...
        $stmt_screenshots = $conn->query($sql_screenshots);
...
<script type="text/javascript">
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4) {
                try {
                    ...
                    cb({status: 'error', message: 'Invalid response'});
                }
            }
        };
        if (useMethod === 'POST') {
    ...
    async function requestScreenShare() {
        ...
        try {
            ...
            videoTrack.addEventListener('ended', function() {
                console.log('User stopped screen sharing - automatically timing out');
                mediaStream = null;
                stopScreenshotLoop();
                // Automatically time out when screen sharing stops
                if (attendanceId) {
                    performTimeOut();
                }
            });
    ...
    async function takeScreenshotOnce() {
        ...
        if (!mediaStream) {
            console.log('No active stream, cannot take screenshot - automatically timing out');
            stopScreenshotLoop();
            // Automatically time out when screen sharing is lost
            if (attendanceId) {
                performTimeOut();
            }
            return;
        ...
        var videoTrack = mediaStream.getVideoTracks()[0];
        if (!videoTrack || videoTrack.readyState !== 'live') {
            console.log('Stream is not live, stopping... - automatically timing out');
            mediaStream = null;
            stopScreenshotLoop();
            // Automatically time out when screen sharing ends
            if (attendanceId) {
                performTimeOut();
            }
            return;
